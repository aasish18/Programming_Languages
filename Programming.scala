...
abstract class Tree[+T] {
  def map[Element](function:T=>Element): Tree[Element]
} // This definition should be roughly 3 lines, including closing "}"
...
case class Leaf[+T](x: T) extends Tree[T]{
  val leaf: T = x
  override def toString() = "Leaf("+leaf+")"
  override def map[Element](function:T=>Element): Leaf[Element] = Leaf(function(this.leaf))
} // Roughly 4 lines
...
case class Node[+T](x: T, children: List[Tree[T]]) extends Tree[T]{
  val leaf: T = x
  override def toString() = "Node("+leaf+","+children.map(a=>a.toString())+")"
  override def map[Element](function:T=>Element): Node[Element] = Node(function(leaf),children.map(child=>child.map(function)))
}  // Roughly 4 lines
...
class A(x:Int) {
  ...
  override def toString() = "A("+this.value+")"
}  // Roughly 5 lines
...
object Part2 {
  ...
  def breadthFirst[T](t: Tree[T]): List[T] = {
    def breadth(tr:List[Tree[T]], value: List[T]): List[T] = tr match {
      case List() => value
      case Leaf(x)::rest => breadth(rest, value++List(x))
      case Node(y, children)::rest => breadth(rest++children, value++List(y))
      case _ => List()
    }
    return breadth(List(t), List())
  } // Roughly 9 lines
  ...
  def reduce[Element,T](f:(T,Element)=>Element, b: Element, L: List[T]): Element = L match{
    case List()=> b
    case x::y => f(x, reduce(f, b, y))
  }  // Roughly 4 lines
  ...
  def sumTreeA(tree: Tree[A]): A = tree match{
    case Node(value,children) => reduce((a:A, b:A) => a + b, value, children.map(child=>sumTreeA(child)))
    case Leaf(x) => x
  }// Roughly 4 lines
