...
interface LessThanOrEqualTo<T>{
    boolean lessThan(T lt);
    boolean equalTo(T et);
}
...
interface SetOf<T extends LessThanOrEqualTo<T>> extends Iterable<T>, LessThanOrEqualTo<SetOf<T>>{
    ...
    void add(T object);
    boolean empty();
    boolean member(T object);
    boolean subset(SetOf<T> object);
    SetOf<T> union(SetOf<T> object);
    SetOf<T> intersection(SetOf<T> object);
    SetOf<T> difference(SetOf<T> object);
}
...
class ASet<T extends LessThanOrEqualTo<T>> implements SetOf<T>{
    ...
    public boolean lessThan(SetOf<T> set){
        return (this.subset(set) && !this.equalTo(set));
    }

    public boolean equalTo(SetOf<T> set){
    ...
    public boolean member(T obj){
        for (T iterator: this.object){
            if (iterator.equalTo(obj)){
                return true;}
        }
        return false;
    }

    public boolean subset(SetOf<T> value){
        for(T iterator: this.object){
            int isSubset = 0;
            for (T iterator1: value){
                if (iterator.equalTo(iterator1)){
                    isSubset = 1;}
            }
            if (isSubset==0){
                return false;}
    ...
    public SetOf<T> difference(SetOf<T> value){
        SetOf<T> set = new ASet<T>();
        for (T iterator: this.object){
            if (!value.member(iterator)){
                set.add(iterator);
            }
        }
        return set;
    ...
    public SetOf<T> intersection(SetOf<T> value){
        SetOf<T> set = new ASet<T>();
        for(T iterator:this.object){
            if (value.member(iterator)){
                set.add(iterator);}
    ...
    public SetOf<T> union(SetOf<T> value){
        SetOf<T> set = new ASet<T>();
        for (T iterator: this.object){
            set.add(iterator);}
        for (T iterator1: value){
...
class A implements LessThanOrEqualTo<A>{
    ...
    public boolean equalTo(A object){
        if (this.value==object.value)
            return true;
        return false;
    }

    public boolean lessThan(A object){
        if (this.value<object.value)
            return true;
        return false;
    }
...
class B extends A{
    ...
    public String toString(){
        return "B[" + (this.value1) + "," + (this.value2) + "]";
    }
}
...
class Part1 {
    ...
    static <T extends LessThanOrEqualTo<T>> SetOf<SetOf<T>> powerSet(SetOf<T> value){

        if (value.empty()){
            SetOf<SetOf<T>> set1 = new ASet<>();
            set1.add(value);
            return set1;
        } else {
            SetOf<T> x = new ASet<>();
            SetOf<SetOf<T>> member = new ASet<>();
            T element = value.get();
            x.add(element);
            SetOf<T> xs = value.difference(x);
            SetOf<SetOf<T>> powerSetxs = powerSet(xs);
            for(SetOf<T> iterator: powerSetxs){
                SetOf<T> y = iterator.union(x);
                member.add(y);
            }
            return powerSetxs.union(member);
        }
    }
